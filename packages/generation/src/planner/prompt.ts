...
export function buildSystemPrompt(input: TeacherInput, version: PromptVersion = PROMPT_VERSION): string {
  return [
    ...
    `- assetEligibility: at most ${MAX_GENERATED_ASSETS} entries, kind in {${GENERATABLE_ASSET_KINDS.join(', ')}}: portrait -> a stakeholder id, landmark -> a room id, prop -> an evidence id, one per entity. Include one portrait for EVERY stakeholder before spending remaining slots on the most story-specific landmarks and props. Portrait prompts must name concrete identity cues (age, hair, facial hair, period/culturally appropriate clothing and expression) rather than a generic role. Landmark and prop prompts must name the place- and period-specific architecture, materials or physical details that distinguish them. Never request terrain, tiles, UI or backgrounds.`,
