...
export function buildSystemPrompt(input: TeacherInput, version: PromptVersion = PROMPT_VERSION): string {
  return [
    ...
    `- assetEligibility: kind in {${GENERATABLE_ASSET_KINDS.join(', ')}}: portrait -> a stakeholder id, landmark -> a room id with a non-null physical landmark, prop -> an evidence id, one per entity. Include one portrait for EVERY stakeholder and images for physical landmarks and evidence props that players can encounter. There is no fixed image count limit. Portrait prompts must name concrete identity cues (age, hair, facial hair, period/culturally appropriate clothing and expression) rather than a generic role. Landmark and prop prompts must name the place- and period-specific materials and physical details that distinguish them. A generated landmark becomes a solid, interactive 2x2 tile fixture on the gameplay map; describe one object whose silhouette is readable at 32x32 pixels. Never request terrain, backgrounds, UI or framed pictures.`,
