...
import { AMBIENT_OVERLAYS, GENERATABLE_ASSET_KINDS, MAP_THEMES, ROOM_KINDS } from '../spec/catalogue'
...
export function buildSystemPrompt(input: TeacherInput, version: PromptVersion = PROMPT_VERSION): string {
  return [
    ...
    `- assetEligibility: kind in {${GENERATABLE_ASSET_KINDS.join(', ')}}: portrait -> a stakeholder id, landmark -> a room id, prop -> an evidence id, one per entity. Include one portrait for EVERY stakeholder and images for story-specific landmarks and props wherever they help the teacher preview the adventure. There is no fixed image count limit. Portrait prompts must name concrete identity cues (age, hair, facial hair, period/culturally appropriate clothing and expression) rather than a generic role. Landmark and prop prompts must name the place- and period-specific architecture, materials or physical details that distinguish them. Gameplay landmarks are assembled from curated map tiles rather than generated pictures. Never request terrain, tiles, UI or backgrounds.`,
