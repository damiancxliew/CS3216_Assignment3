...
export function buildImagePrompt(entry: AssetEligibility, spec: AdventureSpec): string {
  ...
  const mapStyle = entry.kind === 'portrait'
    ? null
    : entry.kind === 'cover'
      ? `Use colors inspired by the ${stage?.mapTheme ?? 'classic'} setting: ${THEME_PALETTES[stage?.mapTheme ?? 'classic']}, with natural tonal variation and smooth color transitions.`
