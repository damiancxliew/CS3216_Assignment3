...
export function AdventureCover({ src }: { src: string | null }) {
  ...
  return (
    <div className="absolute inset-0 overflow-hidden bg-record-wash" aria-hidden="true">
      ...
      {src && failedSrc !== src ? (
        ...
        <img ref={imageRef} src={src} alt="" loading="lazy" decoding="async" onError={() => setFailedSrc(src)} className="absolute inset-0 h-full w-full object-cover transition-transform duration-700 motion-safe:group-hover:scale-105" />
      ) : null}
    </div>
  );
}
