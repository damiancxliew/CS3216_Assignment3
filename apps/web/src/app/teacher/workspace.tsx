...
export function TeacherWorkspace({ adventures, resume }: { adventures: React.ReactNode; resume?: BriefState }) {
  ...
  const panel = (
    <section
      className={
        composing
          ? "mx-auto flex min-h-0 w-full max-w-4xl flex-1 flex-col gap-5 rounded-surface border border-line bg-surface p-6 lg:max-h-full lg:overflow-y-auto"
          : "flex min-h-0 flex-col gap-5 rounded-surface border border-line bg-surface p-6 lg:max-h-full lg:overflow-y-auto"
      }
    >
