...
function Artwork({
  ...
  kind: "portrait" | "landmark" | "prop" | "sprite" | "cover" | "cutscene";
  alt: string;
      ...
      {view === "artwork" ? <Section title="Artwork">
        <div className="flex flex-col gap-4">
          ...
          <p className="text-base text-ink">
            ...
            <span className="text-muted"> · {artworkKindCount(dossier.artwork.filter((item) => item.kind === "cover").length, "cover art")}, {artworkKindCount(dossier.artwork.filter((item) => item.kind === "cutscene").length, "stage opening")}, {artworkKindCount(dossier.artwork.filter((item) => item.kind === "portrait").length, "portrait")}, {artworkKindCount(dossier.artwork.filter((item) => item.kind === "sprite").length, "sprite")}, {artworkKindCount(dossier.artwork.filter((item) => item.kind === "landmark").length, "place")}, {artworkKindCount(dossier.artwork.filter((item) => item.kind === "prop").length, "object")}</span>
          </p>
          <ArtworkProgress
                ...
                <li key={item.id} className="flex flex-col gap-3 rounded-surface border border-line bg-surface p-3">
                  <div className="flex gap-3">
                    ...
                    <div className="flex min-w-0 flex-col gap-1">
                      <p className="font-semibold text-ink">{item.name}</p>
                      <p className="text-sm text-muted">{item.kind === "cover" ? "Cover art" : item.kind === "cutscene" ? "Stage opening" : item.kind === "prop" ? "Object" : item.kind === "landmark" ? "Place" : item.kind === "sprite" ? "Walking sprite" : "Portrait"} · {item.imageStatus === "generated" ? "Ready" : item.imageStatus === "pending" ? "Pending" : item.imageStatus === "failed" ? "Failed" : "Placeholder"}</p>
