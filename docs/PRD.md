...
| D4 | **Hybrid assets with a hard line: generation is only for scene/story-specific entities** — character portraits, named landmarks, story props — and only when no curated asset fits. Terrain, structural and UI art (grass, wall, floor, water, path, doors, panels, icons) is curated. Generated images have no fixed count limit, are cached by prompt hash, and never block publish. Gameplay landmarks are built from solid map tiles; their generated images remain in the teacher preview. | Revised 24 Sep at the teacher's request. Tile-built fixtures preserve map readability and pathfinding; image eligibility still requires a named entity and allows at most one image per entity. |
...
## 10. Decisions log (all resolved 20 Sep)
...
4. ~~How many generated images per adventure, and are they blocking for publish?~~ **Resolved 20 Sep
   — scene/story-specific entities only (characters, landmarks, props), never terrain or UI; cached,
   non-blocking for publish. The eight-image cap was removed 24 Sep** (D4, FR-6/FR-6a/FR-6b). Owner: Di Heng.
