...
it.runIf(runBrowser)("plays a student stage by keyboard with pending dialogue, evidence, decisions, and resume", async () => {
  ...
  try {
    ...
    expect(farquharBody.state.stage.objectives.some((objective: any) => objective.id === "obj-hear-farquhar" && objective.met)).toBe(false);
    await expect(page.getByRole("textbox", { name: "What you say" })).toBeEnabled();
    await page.getByRole("textbox", { name: "What you say" }).focus();
    await page.keyboard.type("What evidence leads you to that assessment?");
    const farquharFollowUp = page.waitForResponse((response) => response.url().includes(`/api/attempt/${attemptId}/message`) && response.request().method() === "POST");
    await page.keyboard.press("Enter");
    const farquharFollowUpBody = await (await farquharFollowUp).json();
    expect(farquharFollowUpBody.accepted).toBe(true);
    expect(farquharFollowUpBody.state.stage.objectives.some((objective: any) => objective.id === "obj-hear-farquhar" && objective.met)).toBe(true);
    ...
    expect(temenggongBody.state.stage.objectives.some((objective: any) => objective.id === "obj-meet-temenggong" && objective.met)).toBe(false);
    await expect(page.getByRole("textbox", { name: "What you say" })).toBeEnabled();
    await page.getByRole("textbox", { name: "What you say" }).focus();
    await page.keyboard.type("What would you need before accepting an agreement?");
    const temenggongFollowUp = page.waitForResponse((response) => response.url().includes(`/api/attempt/${attemptId}/message`) && response.request().method() === "POST");
    await page.keyboard.press("Enter");
    const temenggongFollowUpBody = await (await temenggongFollowUp).json();
    expect(temenggongFollowUpBody.accepted).toBe(true);
    expect(temenggongFollowUpBody.state.stage.objectives.some((objective: any) => objective.id === "obj-meet-temenggong" && objective.met)).toBe(true);
