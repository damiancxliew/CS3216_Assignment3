-- Attempt-scoped Resolver options (FR-13).
--
-- A minted option is deliberately not part of decision_option: that table is
-- frozen with the published spec and shared by every attempt on that version.
create table minted_option (
  ...
  unique (attempt_id, spec_id)
);

alter table minted_option enable row level security;

revoke all on minted_option from anon, authenticated;
grant select (id, attempt_id, stage_id, spec_id, label)
  on minted_option to authenticated;

create policy minted_option_select on minted_option
  for select to authenticated
  using (owns_attempt(attempt_id) or teaches_attempt(attempt_id));

alter table stage_commitment
  add column minted_option_id uuid references minted_option (id) on delete set null;

alter table stage_commitment
  add constraint stage_commitment_single_option check (
    not (option_id is not null and minted_option_id is not null)
  );

drop function if exists save_play_turn(uuid, bigint, text, jsonb, jsonb, jsonb, jsonb, jsonb, uuid, text);

create function save_play_turn(
  p_attempt_id uuid,
...
begin
  ...
  if v_attempt.status <> 'active' then
    raise exception 'attempt is no longer active' using errcode = '40001';
  end if;
  ...
  if p_expected_revision is null or p_expected_revision <> v_revision then
    raise exception 'attempt runtime revision conflict' using errcode = '40001';
  end if;
