...
begin
  ...
  if jsonb_typeof(p_snapshot) is distinct from 'object'
     ...
     or jsonb_typeof(p_commitments) is distinct from 'array' then
  ...
  for r in select * from jsonb_to_recordset(p_commitments) as c(
    ...
    if r.stage_id is null or not (r.stage_id = any(v_stage_ids))
       ...
       or (r.option_id is not null and not exists (select 1 from decision_option where id = r.option_id and stage_id = r.stage_id)) then
...
revoke all on function save_play_turn(uuid, bigint, text, jsonb, jsonb, jsonb, jsonb, jsonb, uuid, text)
  from public, anon, authenticated;
grant execute on function save_play_turn(uuid, bigint, text, jsonb, jsonb, jsonb, jsonb, jsonb, uuid, text)
  to service_role;
